...
// Reporter fetches all approved ads (only if reporter is verified)
const getApprovedAdsForReporter = async (req, res) => {
  try {
    const reporterId = req.user._id;

    // 1. Check reporter is verified
    if (!req.user.isVerified) {
      return res.status(403).json({
        success: false,
        message: "You must be a verified reporter to access approved ads",
      });
    }

    // 2. Find all ads reporter already responded to
    ...
    // 3. Fetch all approved ads EXCEPT those already accepted/rejected by this reporter
    const ads = await RaiseYourVoice.find({
      status: "approved",
    ...
    console.error("Error fetching approved ads for reporter:", error);
    res.status(500).json({
      success: false,
      message: "Server error while fetching approved ads",
    });
  }
};

// Reporter accepts an ad
const reporterAcceptRyvAd = async (req, res) => {
  try {
    ...
    // Create record with accepted status only
    const acceptance = new raiseYourVoiceProof({
      adId,
      reporterId,
      status: "accepted", // nothing else for now
      proof: false,
    });
...
const getRejectedAdsForReporter = async (req, res) => {
  try {
    ...
    // console.log("Fetching rejected ads for reporter ID:", reporterId);
